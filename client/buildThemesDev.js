...
console.log("🚀 ~ file: buildThemesDev.js ~ line 4 ~ process.env", process.env)

const themeArgv = process.argv[2];
const countryArgv = process.argv[3];
//console.log(`theme arg: ${themeArgv} country arg ${countryArgv}`);
const brands = {
...
const buildTheme = (brand, country) => `set brand=${brand}&& set country=${country}&& gulp build-theme:dev`;
function PerformTask(theme, country) {
  const command = buildTheme(theme, country);
  console.log(`set mode=theme&& ${command}`);
  execSync(`set mode=theme&& ${command} --development`, { stdio: [0, 1, 2] });
}
(function () {
  ...
  execSync(`set mode=theme&& gulp build-script:dev --development`, { stdio: [0, 1, 2] });
})();
